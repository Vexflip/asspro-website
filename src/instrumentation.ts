...
export async function register() {
  // Only the Node.js runtime needs SMTP, and never validate during `next build`
  // — the mail credentials aren't present at build time and must not fail it.
  if (
  ...
  if (missing.length > 0) {
    ...
    // In production a missing SMTP config is a deploy error — fail fast so it is
